...
func CreatePullRequest(client *github.Client, org, repo, title, body, baseRepoOwner, baseBranch, headRepoOwner, headBranch, currentRevision, latestRevision string, patchApplySucceeded bool) error {
	...
	if len(pullRequests) > 0 {
		...
		if !patchApplySucceeded {
			...
			for _, comment := range pullRequestComments {
				if comment.Body == github.String(constants.PatchesCommentBody) {
